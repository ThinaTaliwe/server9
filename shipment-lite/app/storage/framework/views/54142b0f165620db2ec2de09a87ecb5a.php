...
<head>
    ...
    <title>PortHub - South African Maritime Directory</title>
    ...
    <style>
        * {
        ...
        .btn-primary:hover {
            transform: translateY(-2px);
            box-shadow: 0 4px 12px rgba(0, 119, 73, 0.4);
        }
        ...
        .sa-flag .green { background: #007749; }
        .sa-flag .yellow { background: #FFB81C; }
        .sa-flag .black { background: #000000; }
    </style>
</head>
<body>
    <div class="container">
        <!-- Header -->
        <header class="header">
            <img src="https://image.qwenlm.ai/public_source/75ad6831-1a69-4a8a-ac03-9979f8ddcd76/1d4bca349-a2b1-4cdf-a441-f75794dc09dc.png" alt="Shipping Port" class="header-image">
            <div class="header-content">
                ...
                <p>Comprehensive maritime directory for South African ports, shipping lines, and customs clearing agents. Your complete resource for logistics and trade operations.</p>
                <div class="api-badge">
                    <span class="dot"></span>
        ...
        <div class="stats-grid" id="statsGrid">
            ...
            <div class="stat-card">
                ...
                <div class="label" id="stat2Label">Annual TEU</div>
            </div>
            <div class="stat-card">
                ...
                <div class="map-panel">
                    ...
                    <div class="map-info">
                        ...
                        <div id="routeInfo" style="display: none; margin-top: 16px;">
                            <h4 style="font-size: 0.95rem; font-weight: 600; margin-bottom: 8px; color: var(--dark);">🗺️ Active Shipping Routes</h4>
    ...
    <script>
        ...
        const shippingRoutes = {
            // Asia Route: Durban -> Indian Ocean -> Asia
            "asia-route": {
                ...
                path: [
                    ...
                    [30.0, 100.0],
                    [35.0, 110.0],
                    [40.0, 120.0] // Towards Asia
                ]
            },
            // Europe Route: Cape Town -> Atlantic -> Europe
            "europe-route": {
                name: "Europe Atlantic Lane",
                type: "International",
                color: "#dc2626",
                path: [
                    [-33.9067, 18.4233], // Cape Town
                    [-30.0, 15.0],
                    [-25.0, 10.0],
                    [-20.0, 5.0],
                    [-15.0, 0.0],
                    ...
                    [30.0, -35.0],
                    [40.0, -40.0],
                    [50.0, -45.0] // Towards Europe
                ]
            },
            // Intra-Africa: Durban -> Maputo -> Mombasa
            "africa-route": {
            ...
            // Americas: Cape Town -> South Atlantic -> Americas
            "americas-route": {
                ...
                path: [
                    ...
                    [-42.0, -40.0],
                    [-40.0, -50.0],
                    [-35.0, -60.0] // Towards South America
                ]
            }
        };
        ...
        let map = null;
        let markers = [];
        let routeLayers = []; // Store route polyline layers
        ...
        function switchTab(tab) {
            ...
            // Clear routes when switching tabs
            clearRoutes();
        ...
        function updateFilters() {
            ...
            if (currentTab === 'ports') {
                filter1.innerHTML = '<option value="">All Provinces</option>' +
                    [...new Set(allData.map(p => p.province))].sort().map(p => 
                        `<option value="${p}">${p}</option>`
                    ).join('');
                filter2.innerHTML = '<option value="">All Types</option>' +
                    [...new Set(allData.map(p => p.type))].sort().map(t => 
                        `<option value="${t}">${t}</option>`
                    ).join('');
            } else if (currentTab === 'shipping') {
                filter1.innerHTML = '<option value="">All Types</option>' +
                    [...new Set(allData.map(s => s.type))].sort().map(t => 
                        `<option value="${t}">${t}</option>`
                    ).join('');
                filter2.innerHTML = '<option value="">All Status</option>' +
                    [...new Set(allData.map(s => s.status))].sort().map(s => 
                        `<option value="${s}">${s.charAt(0).toUpperCase() + s.slice(1)}</option>`
                    ).join('');
            } else if (currentTab === 'agents') {
                filter1.innerHTML = '<option value="">All Types</option>' +
                    [...new Set(allData.map(a => a.type))].sort().map(t => 
                        `<option value="${t}">${t}</option>`
                    ).join('');
                filter2.innerHTML = '<option value="">All Cities</option>' +
                    ['Johannesburg', 'Durban', 'Cape Town', 'Gqeberha', 'Pretoria', 'East London'].map(c => 
        ...
        function updateMapMarkers() {
            ...
            markers = [];
            clearRoutes(); // Clear routes when markers update
            ...
            filteredData.forEach(item => {
                ...
                const marker = L.marker([item.lat, item.lon], { icon: customIcon })
                    .addTo(map)
                    .bindPopup(`<b>${escapeHtml(item.name)}</b><br>${currentTab === 'ports' ? item.province : item.saOffice || item.headquarters}`);
                markers.push(marker);
            });
        ...
        function drawRoutes(item) {
            ...
            // Fit bounds to include routes if possible
        ...
        function selectItem(index) {
            ...
            if (currentTab === 'ports') {
                ...
                // Draw Routes for Shipping Lines
                drawRoutes(item);
            } else if (currentTab === 'agents') {
                statsDiv.innerHTML = `
                    <div class="info-stat">
...
</html><?php /**PATH /app/resources/views/ShipmentSA.blade.php ENDPATH**/ ?>