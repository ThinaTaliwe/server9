...
btnLoadMeta.addEventListener('click', async () => {
  ...
  const selBu = document.getElementById('selBu');
    fillSelectWithObjects(
      selBu,
      buRows,
      "Select BU…",
      (r) => `${r.bu_name ?? r.name ?? r.short_code ?? 'BU'} (id: ${r.id})`
    );

    const bu1 = buRows.find(x => Number(x.id) === 1);
    if (bu1) {
      selBu.value = String(bu1.id);
    }
    ...
    fillSelectWithObjects(
      ...
      (r) => `${r.name ?? 'Shipment Type'} (id: ${r.id})`
    );
    ...
    fillSelectWithObjects(
      ...
      (r) => `${r.name ?? 'Instruction Type'} (id: ${r.id})`
    );
...
<?php /**PATH /app/resources/views/bfrn/shipcreate.blade.php ENDPATH**/ ?>